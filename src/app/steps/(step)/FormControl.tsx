...
export const FormControl = ({ children }: BTypes.FCChildren) => {
  ...
  const { stepsValues } = useSteps<'name' | 'email' | 'phone'>();
  const { next, currStep, stepsCompleted } = useStepsNavigation();

  function handleSubmit(ev: React.FormEvent<HTMLFormElement>) {
    ev.preventDefault();
    if ((stepsValues[1].phone?.value?.length || 0) <= 8 && step === 1) return;
    next();
  }
  ...
  return (
    <form onSubmit={handleSubmit}>
      {children}
      <section className="flex fixed bottom-8 right-8 self-end h-12">
