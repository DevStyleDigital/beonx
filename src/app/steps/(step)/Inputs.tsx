		...
		<>
			{(inputs as (Omit<InputType, "step"> & { min?: number; max?: number })[])
				...
				.map((input, i, arr) => {
					...
					const Comp =
						INPUTS_COMPONENTS[input.type as keyof typeof INPUTS_COMPONENTS];
					...
					return (
						<Comp
							...
							disabled={
								stepByStep &&
								i !== 0 &&
								!stepsValues?.[step]?.[lastInputId]?.value
							}
