...
export const Chart = () => {
	...
	function getData() {
		...
		const toIncrement = [
			...
			newDataValues.reduce(
				(acc, curr, i) => (!curr && i > 0 ? acc + 10 : acc),
				0,
			),
			newDataValues.reduce(
				(acc, curr, i) => (!curr && i > 1 ? acc + 10 : acc),
				0,
			),
			newDataValues.reduce(
				(acc, curr, i) => (!curr && i > 2 ? acc + 10 : acc),
				0,
			),
		];
		...
		setValues([
			`$ ${formatToCurrency(currRevenue)}`,
			...valuesRms.map(formatToCurrency),
		]);
