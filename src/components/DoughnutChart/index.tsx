...
> = ({
  ...
  const strokeWidthNumber = Number(strokeWidth.replaceAll(/(\d*[.]?\d*).*/g, '$1'));

  const size = (additionalSize: number = 0) =>
...
const DoughnutChartRoot: BTypes.FC<
  ...
  {},
  false
> = ({ defaultPercent, percentRange, textClassName, contentSize = '3rem', children }) => {
  return (
    <div
    ...
    >
      ...
      >
        ...
        {percentRange && (
          <span className="text-lg font-medium text-gray-600">0-{percentRange}</span>
        )}
      </span>
    </div>
  );
