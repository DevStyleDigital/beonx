...
const SidebarLink: BTypes.FC<
  Link & { className?: string; pathname: string | null },
  {},
  false,
  false
      ...
      >
        <div className="flex items-center justify-center mt-8">
          <div className="flex items-center">
            <span className="text-2xl font-semibold text-white">GB-Dashboard</span>
          </div>
        </div>
        <nav
          ...
          {links.map((link: Link) =>
            link.child ? (
              <Fragment key={link.id}>
                <span
                  className="flex items-center px-4 py-2 mt-4 duration-200 text-gray-500"
                  role="separator"
                >
                  - {link.content}
                </span>
                {link.child.map((linkChild: Link, i) => (
                  <SidebarLink
                    pathname={pathname}
                    className={i === 0 ? '!mt-0' : ''}
                    key={linkChild.id}
