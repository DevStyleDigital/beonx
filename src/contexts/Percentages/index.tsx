...
const Context = createContext<StepsNavigationContext>(
	{} as StepsNavigationContext,
);
...
export const PercentagesProvider: BTypes.FC<{}, {}, false> = ({ children }) => {
		...
		(group, value) => {
			...
			setPercentages((prev) => {
				...
				const allPercentagesValueSum = Object.entries(prev).reduce(
					(acc, [key, curr]) =>
						key !== "total" && key !== group ? acc + curr : acc,
					0,
				);
				...
				return {
					...
					[group]: newPercentage,
					total: Math.floor(
						(allPercentagesValueSum + newPercentage) /
	...
	return (
		<Context.Provider
			value={{ percentages, groups: GROUPS, updateGroupPercentage }}
		>
			{children}
		</Context.Provider>
	);
